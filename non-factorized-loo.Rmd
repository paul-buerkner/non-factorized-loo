...
This is straightforward for *factorizable* models in which response values are
...
response values are not conditionally independent. When there is
...
Sundararajan and Keerthi's results also generalize to the case of
an arbitrary invertible covariance matrix $C$. For such models, 
...
which the Pareto $k$
[diagnostic](http://mc-stan.org/loo/reference/pareto-k-diagnostic.html) 
(Vehtari et al, 2017b) indicates an unstable approximation, we need to consider
how we might to do _exact_ leave-one-out CV for a non-factorizable model. In the
case of a Gaussian process that has the marginalization property, we could just
drop the one row and column of $C$ corresponding to the held out out
observation. This does not hold in general for multivariate normal models,
however, and to keep the original prior we may need to maintain the full
...
is necessary in order to validate the approximate LOO-CV results.
...
* Code &copy; 2018, Aki Vehtari, Paul Buerkner and Jonah Gabry, licensed under BSD-3.
* Text &copy; 2018, Aki Vehtari, Paul Buerkner and Jonah Gabry, licensed under CC-BY-NC 4.0.
