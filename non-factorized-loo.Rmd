...
The results of approximate and exact LOO-CV are rather similar but not as close as we would expect **if** there were no problematic observations. We can investigate this issue more closely by plotting the approximate against the exact pointwise elpds.
...
Summing over the non-problematic observations only, reveals practically equivalent results;

````{r}
sum(psis_looic$pointwise[-4, "elpd_loo"])
sum(exact_elpds[-4])
```

Accordingly, the differences found when including all observations do not indicate an eror in our implementation of the approximate LOO-CV but rather are due to violations of its assumptions.
