...
$$
which corresponds to the following log probability density function coded in
**Stan**:
...
real normal_lagsar_lpdf(vector y, vector mu, real sigma, 
                        real rho, matrix W) {
  int N = rows(y);
  matrix[N,N] W_new = diag_matrix(rep_vector(1, N)) - rho * W;
  vector[N] half_pred = W_new * (y - mdivide_left(W_new, mu));
...
The code above fits the model in **Stan** using a log probability density
function equivalent to the `normal_lagsar_lpdf` function we defined above. In
the summary output below we see that both higher income and higher housing value
predict *lower* crime rates in the neighborhood. Moreover, there seems to be
substantial spatial correlation between adjacent neighborhoods, as indicated by
the `lagsar` parameter.
...
for (s in 1:S) {
  ...
  IB <- diag(N) - p$lagsar * fit$autocor$W
  Cinv <- t(IB) %*% IB / p$sigma^2
...
for (i in seq_len(N)) {
  ...
  for (s in seq_len(S)) {
    ...
    IB <- diag(N) - p$lagsar * fit_i$autocor$W
    Cinv <- t(IB) %*% IB / p$sigma^2
...
the distribution the implied response values for the left-out observation to the
...
In the final step, we compute the expected log predictive density (ELPD) based
on the exact LOO-CV and compare it to the approximate PSIS-LOO result computed
earlier.
...
From this we can conclude that the differences found when including *all*
observations do not indicate a bug in our implementation of the approximate
