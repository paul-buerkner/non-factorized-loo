...
$$
 p(y_i|y_{-i}) \approx 
 \sum_{s=1}^S \frac{ p(y_i|y_{-i}, \theta^{(s)}) w^{(s)}}{w^{(s)}},
$$
$w^{(s)}$ are importance weights. We first compute raw importance ratios
$$
...
$w^{(s)}$. The resulting approximation is called Pareto-smoothed
...
$$
where $\theta_{-i}^{(s)}$ $(s=1,\ldots,S)$ are draws from
